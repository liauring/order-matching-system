...
const BSLogicMap = new MatchLogic()
...
async function matchLogic(orderFromQueue) {
  ...
  let { BS } = order
  let dealerInfo = BSDealerInfoMap[BS](order.price)
  let dealerProvider = DealerProvider(dealerInfo, order.symbol, CacheProvider)
  let executionUpdater = ExecutionUpdater(QueueProvider, CacheProvider)
  let matchLogic = new BSLogicMap[BS](order, dealerProvider, QueueProvider, CacheProvider, executionUpdater)
